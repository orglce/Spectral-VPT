...
float get_transmission_spectrum_value(float wavelength) {
    // implement more spectra
    // add suppport for tabelaric spectra as well
    return 1.0 - get_gaussian_value(wavelength, 1.0, 550.0, 20.0);
}

vec3 wavelength_to_RGB(float wavelength) {
    float x = get_gaussian_value(wavelength, 159.0 / 268.0, 597.0, 35.0) + get_gaussian_value(wavelength, 56.0 / 268.0, 446.0, 20.0);
    float y;
    float z;
    if (wavelength < 557.0) {
        y = get_gaussian_value(wavelength, 151.0 / 268.0, 554.0, 39.0);
    } else {
        y = get_gaussian_value(wavelength, 151.0 / 268.0, 557.0, 43.0);
    }
    if (wavelength < 445.0) {
        z = get_gaussian_value(wavelength, 268.0 / 268.0, 445.0, 18.0);
    } else {
        z = get_gaussian_value(wavelength, 268.0 / 268.0, 447.0, 28.0);
    }
...
//        float refraction_index = rgbaToFloat(volumeSample);
        ...
        if (any(greaterThan(photon.position, vec3(1))) || any(lessThan(photon.position, vec3(0)))) {
            ...
            float photonRadiance = photon.wavelengthTransmittance * envSample.r;
...
//            photon.direction = sampleHenyeyGreenstein(state, uAnisotropy, photon.direction);
    ...
    vec3 color = mix(vec3(1.0), wavelength_to_RGB(photon.wavelength), t);
    oRadiance = vec4(color, float(photon.samples));
...
float get_D65_spectrum_value(float wavelength) {
    return D65[int(wavelength)] / 118.0;
}
...
//    unprojectRand(state, vPosition, uMvpInverseMatrix, uInverseResolution, uBlur, from, to);
    ...
    photon.samples = 0u;
    photon.wavelength = random_uniform(state) * (720.0 - 320.0) + 320.0;
    photon.wavelengthRadiance = get_D65_spectrum_value(photon.wavelength);
    photon.wavelengthTransmittance = 1.0;
    oPosition = vec4(photon.position, 0);
    oDirection = vec4(photon.direction, float(photon.bounces));
    oTransmittance = vec4(photon.transmittance, float(photon.wavelength));
